...
The filter pass runs **after** the existing qualification pass, so table names
are already schema-qualified and CTE references already de-qualified. It is a
separate tree-walk, independent of `qualify_node`.
